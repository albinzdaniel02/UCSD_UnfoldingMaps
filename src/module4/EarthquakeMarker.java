...
public abstract class EarthquakeMarker extends SimplePointMarker
{
	...
	public void draw(PGraphics pg, float x, float y) {
		...
		int factor=5;
		if(this.getPast()==1)
			pg.line(x,y-2*factor,x,y+2*factor);//verticle line for past hour
		else if(this.getPast()==2)
			pg.line(x-2*factor,y,x+2*factor,y);//horizontal line for past day
		else if(this.getPast()==3){
			pg.line(x,y-2*factor,x,y+2*factor);
			pg.line(x-2*factor,y,x+2*factor,y);
		}
		else if(this.getPast()==4) {
			pg.stroke(50, 252, 61);
			pg.line(x,y-2*factor,x,y+2*factor);
			pg.line(x-2*factor,y,x+2*factor,y);
		}
